...
#define AUTO_UPDATE_CHECKING_INTERVAL 86400 // 1 day
...
@implementation UniCall {
    ...
    BOOL hasStartedSettingUp_;
    BOOL isInProcessOfSettingUp_;
    BOOL hasSettingUpSucceeded;
    BOOL isNewUser_;
}
...
- (id)init
{
    ...
    if (self) {
        ...
        if (!version || [VERSION doubleValue] > [version doubleValue]) {
            // user has upgraded to a new version
            // N.B. !version ensures that users upgrading from v5.31 to v6.0
            // will go through the new setup process
            hasStartedSettingUp_ = NO;
        ...
        updater_ = [[Updater alloc] init:self];
    }
    return self;
}
...
#pragma mark Piggyback Actions
        ...
        if (!rawAutoUpdateCheckingEnabledStatus || [rawAutoUpdateCheckingEnabledStatus boolValue]) {
            NSDate *autoUpdateCheckingLastStartTime = config_[@"autoUpdateCheckingLastStartTime"];
            if (!autoUpdateCheckingLastStartTime || [[NSDate date] timeIntervalSince1970] - [autoUpdateCheckingLastStartTime timeIntervalSince1970] >= AUTO_UPDATE_CHECKING_INTERVAL) {
                [updater_ checkForUpdateAndDeliverNotification:YES withCompletion:nil];
            }
        }
    }
...
#pragma mark Handle Hidden Long Options
                    ...
                    if ([longOption isEqualToString:@"[Update]"]) {
                        if (updater_.responseObject) {
                            [UniCall pushNotificationWithOptions:@{@"title": [NSString stringWithFormat:@"Upgrading Uni Call to v%@...", updater_.responseObject[@"version"]],
                                                                   @"message": @"Click me to check your Console logs for details",
                                                                   @"sound": @"Purr",
                                                                   @"group": @"upgrading",
                                                                   @"activate": @"com.apple.Console"}];
                            system([[NSString stringWithFormat:@"\"%@/updater.bash\" %@ &", [UniCall workingPath], updater_.responseObject[@"sha1"]] UTF8String]);
                        }
                        return YES;
                    }
                    ...
                    if ([@"checkforupdate" hasPrefix:longOption]) {
                        if ([longOption isEqualToString:@"checkforupdate"]) {
                            ...
                            switch (sessionStatus) {
                                case ASIdle: {
                                    sessionStatus = ASChecking;
                                    [updater_ checkForUpdateAndDeliverNotification:NO withCompletion:^(UpdateActionStatus result) {
                                ...
                                case ASHasUpdate:
                                    ...
                                    [results setString:[NSString stringWithFormat:@"\n<?xml version=\"1.0\"?>\n\n<items><item arg=\"[Cmd]%@\" autocomplete=\"-\"><title>Uni Call Option --checkforupdate</title><subtitle>A new version (v%@) of Uni Call is available. Hit return to upgrade</subtitle><icon>succeeded.png</icon></item>", [self xmlSimpleEscape:[NSString stringWithFormat:@"\"%@\" \"--[Update]\"", [self uniCallBasestationPath]]], updater_.responseObject[@"version"]]];
                                    return YES;
                                case ASNoUpdate:
                                    sessionStatus = ASIdle;
                                    [results setString:[NSString stringWithFormat:@"\n<?xml version=\"1.0\"?>\n\n<items><item arg=\"\" autocomplete=\"-\" valid=\"no\"><title>Uni Call Option --checkforupdate</title><subtitle>Your Uni Call (v%@) is already the newest version</subtitle><icon>succeeded.png</icon></item>", VERSION]];
                                    return YES;
                                case ASFailed:
                                    sessionStatus = ASIdle;
...
- (NSString *)uniCallBasestationPath
{
    static NSString *path = nil;
    if (!path) {
        path = [[UniCall workingPath] stringByAppendingString:@"/Uni Call Basestation"];
    }
    return path;
}
